...
class OrderBillController extends PublicController
{
    ...
    )
    {
        ...
        $paidWithUbgxu = $request->get('pay_by_ubgxu');
        $cashbackType = $request->get('cashback_type');

        if ($avaiableUbgxu == 0 && $paidWithUbgxu == '1') {
        ...
        try {
            ...
            if ($customer->presenter_id != null) {
                ...
                if ($presenter != null) {
                    ...
                    //Xử lý giao dịch có xu
                    if ($paidWithUbgxu == 0) { // Trường hợp khách hàng không sử dụng xu để thanh toán
